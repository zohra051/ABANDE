...
// public methods
  ...
  load_level() {
    let key = 'openxum:' + this._that.get_name() + ':level';
...
// private methods
  ...
  _load_win() {
    let key = 'openxum' + this._that.get_name() + ':win';
    let win = 0;
  ...
  _load_elo() {
    let key = 'openxum:' + this._that.get_name() + ':elo';
  ...
  _finish() {
    if (this._engine.is_finished()) {
      ...
      localStorage['openxum:' + this._that.get_name() + ':elo'] = JSON.stringify(new_elo);

      let eloDiff = elo.ai - elo.player;

      if (eloDiff < -40 && new_elo.last < 1) {
        localStorage['openxum:' + this._that.get_name() + ':level'] =
          JSON.stringify(this._level + 10);
        localStorage['openxum:' + this._that.get_name() + ':elo'] = JSON.stringify(
          {
            'ai': new_elo.ai,
