...
// private methods
  ...
  _get_final_choice() {
    let finalChoice = this._root.getChildNodes()[0].get_move();
    let best = this._root.getChildNodes()[0].get_visit_number();

    for (let i = 1; i < this._root.getChildNodes().length; i++) {
      if (this._root.getChildNodes()[i].get_visit_number() > best) {
        best = this._root.getChildNodes()[i].get_visit_number();
        finalChoice = this._root.getChildNodes()[i].get_move();
      }
    }
    return finalChoice;
  }
