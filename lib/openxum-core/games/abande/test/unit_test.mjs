...
{
    console.log("TEST ENGINE : PUT FIRST PIECE : Toutes les couleurs ont bien étais mis à NONE")
}
engine._put_first_piece(color_black,coord_A2);
if( engine._intersections[coord_A2.hash()].getColor() === color_black)
{

    console.log("TEST ENGINE : PUT FIRST PIECE : " + engine._intersections[coord_A2.hash()].getColor() +" (BLACK)\t A2 ");
}
if(engine._black_piece_number === 17)
{
    console.log("TEST ENGINE : PUT FIRST PIECE : Le nombre de pièce noir à bien décrémenté => " + engine.black_piece_number());
}
...
if( engine._intersections[coord_A3.hash()].getColor() === color_white)
{
    ...
    console.log("TEST ENGINE : PUT PIECE : " + engine._intersections[coord_A3.hash()].getColor() +" (WHITE)\t A2 A3 ");
}
if(engine._white_piece_number === 17)
{
    console.log("TEST ENGINE : PUT PIECE : Le nombre de pièce blanche à bien décrémenté => " + engine.white_piece_number() );
}
...
engine._put_piece(color_black,coord_A1);
if( engine._intersections[coord_A1.hash()].getColor() === color_black)
{
    console.log("TEST ENGINE : PUT PIECE : " + engine._intersections[coord_A1.hash()].getColor() +" "+ lib.OpenXum.Abande.Color.display(color_black)+"\t A1 A2 A3 ");
}
...
{
    console.log("TEST ENGINE : PUT PIECE : " + engine._intersections[coord_A3.hash()].getTaillePile() + " La pile à été incrémenté. Le jeton s'est bien placer sur le plateau. Formation d'une pile de 1.");
}
if(engine._is_current_player(color_white) === true)
{
    console.log("TEST ENGINE : PUT PIECE : " + engine._is_current_player(color_white) + " Current couleur est de la couleur => " + engine._current_color);

engine._change_current_player();
if(engine._current_color === color_black)
{
    console.log("TEST ENGINE : PUT PIECE : La couleur du joueur courant à bien étais modifié => " + engine._current_color);
}
/*************************************************************************************************************/
