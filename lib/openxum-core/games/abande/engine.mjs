...
class Engine extends OpenXum.Engine {
  ...
  _put_piece(color, place) {
    if (this._is_current_player(color) && place.is_valid()) {
      if (this._intersections[place.hash()].getColor() === Color.DISPONIBLE && _have_piece(this.current_color())) {
        this._decrease_piece_number(color);
        this._intersections[place.hash()].setColor(color);
        this._intersections[place.hash()].taillePlus();
        this._neighboor_dispo(place);
  ...
  _no_hole(from, to) {
    ...
    let painted = this._paint_board(to);
  ...
  _paint_board(start_hash) {
    ...
    let need_paint = this._get_colored(this._get_neighboor(this._intersections[i]));
