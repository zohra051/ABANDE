...
class Engine extends OpenXum.Engine {
  ...
  _can_capture(from, to) {
    ...
    if ((this._black_piece_number <17 && this._are_opposite_color(from, to) && this._are_neighbour(from, to) && this._no_hole(from, to) && this._Stack_height_verif(from, to))=== true) {
      return true;
    }
    return false;
  }
  ...
  _no_hole(from, to) {
    ...
    let bool = false;
    for (let l = 0; l < neighbour.length; ++l) {
      let my_color = this._intersections[neighbour[l]].getColor();
      if (neighbour[l] !== to.hash() && (my_color === Color.BLACK || my_color === Color.WHITE)) {
