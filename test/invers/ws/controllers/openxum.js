...
exports = module.exports = function (app) {
  return {
    ...
    move: function (req, res) {
      ...
      let obj = jsonfile.readFileSync(file + token + '.json');

      obj.push(move.to_object());
      jsonfile.writeFileSync(file + token + '.json', obj);
      res.status(200).json({});
    },
    ...
    next_move: function (req, res) {
      const token = req.body.id;
      let engine = new OpenXum.Invers.Engine(OpenXum.Invers.GameType.STANDARD, OpenXum.Invers.Color.RED);
      let player = new OpenXum.MCTSPlayer(OpenXum.Invers.Color.RED, engine);
      let obj = jsonfile.readFileSync(file + token + '.json');

      for (let i = 0; i < obj.length; ++i) {
        engine.move(new OpenXum.Invers.Move(obj[i].color, obj[i].letter, obj[i].number, obj[i].position));
      }
    ...
    new_game: function (req, res) {
      ...
      crypto.randomBytes(32, function(err, buffer) {
        ...
        jsonfile.writeFileSync(file + token + '.json', []);
